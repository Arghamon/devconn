...
router.post(
  ...
  (req, res) => {
    ...
    newPost
      .save()
      .then(post => res.json(post))
      .catch(err => res.json({ catchError: err }));
  }
);
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id)
        ...
        .catch(err => {
          res.status(404).json({ noprofilefound: 'Profile not found' });
        }); // findMe catch helll
    });
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    if (!isValid) {
      //if any errors send 400 status with jason object
    ...
    Post.findById(req.params.id)
      .then(post => {
        ...
        post
          ...
          .then(post => res.json(post))
          .catch(err =>
            res.json({ nosave: 'Cannot save comment, call to admin' })
          );
      })
